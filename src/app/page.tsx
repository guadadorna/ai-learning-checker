...
import type { SurveyData } from "./components/SurveyScreen";
import { Semaforo } from "./components/Semaforo";

export type CategoryStatus = "Prohibido" | "Desalentado" | "Permitido" | "Fomentado";
...
export default function Home() {
    ...
    try {
      ...
      if (mode === "pdf" && file) {
        ...
        response = await fetch("/api/analyze", {
          ...
          body: JSON.stringify({ link: link.trim(), surveyData }),
        });
      } else if (mode === "text" && conversation.trim()) {
        response = await fetch("/api/analyze", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ conversation, surveyData }),
        });
      } else {
  ...
  return (
    <>
      {!surveyCompleted && <SurveyScreen onComplete={handleSurveyComplete} />}
                ...
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <h3 className="font-semibold text-slate-700 mb-2">
                      El Semáforo del Uso de AI
                    </h3>
                    <div className="space-y-1.5 text-sm">
                      <div className="flex items-center gap-2">
                        <span className="w-2.5 h-2.5 rounded-full bg-red-500 flex-shrink-0"></span>
                        <span className="text-slate-600"><strong>Prohibido:</strong> Que la AI haga todo el trabajo</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="w-2.5 h-2.5 rounded-full bg-orange-500 flex-shrink-0"></span>
                        <span className="text-slate-600"><strong>Desalentado:</strong> Que la AI haga tu trabajo tedioso</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="w-2.5 h-2.5 rounded-full bg-yellow-400 flex-shrink-0"></span>
                        <span className="text-slate-600"><strong>Permitido:</strong> Ayuda para arrancar o feedback</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="w-2.5 h-2.5 rounded-full bg-green-500 flex-shrink-0"></span>
                        <span className="text-slate-600"><strong>Fomentado:</strong> Aprender o amplificar tu trabajo</span>
                      </div>
                    </div>
                  </div>
                </div>
